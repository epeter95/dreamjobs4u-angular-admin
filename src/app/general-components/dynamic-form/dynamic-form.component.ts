    ...
    if (this.form.valid) {
      ...
      if(this.serviceType == 'create'){
        this.dataService.createData(this.serviceUrl,sendData).subscribe(res=>{
          ...
          if(res.hasOwnProperty('error')){
            if(res.error == 'SequelizeUniqueConstraintError'){
              this.dialog.open(DialogComponent,{
                data: {icon: 'warning', text: 'This element is already exists!'}
              });
              return
            }
            this.dialog.open(DialogComponent,{
              data: {icon: 'warning', text: 'Something went wrong!'}
            });
            return;
          }
          this.tableService.setNextText('refresh');
          this.panelEmitter.emit('closeCreate');
          this.form.reset();
          this.initForm();
          ...
          this.dialog.open(DialogComponent,{
            data: {icon: 'done', text: 'Succesful save!'}
          })
        }, error=>{
          console.log(error);
        });
        ...
        this.dataService.modifyData(this.serviceUrl,this.modifyID,sendData).subscribe(res=>{
          ...
          this.tableService.setNextText('refresh');
          this.panelEmitter.emit('closeModify');
          this.form.reset();
          this.initForm();
          this.fileDatas = new Array();
          if(this.isRefreshNeeded){
          ...
          this.dialog.open(DialogComponent,{
            data: {icon: 'done', text: 'Succesful update!'}
          })
        } ,error=>{
          console.log(error);
        });
      ...
      this.dialog.open(DialogComponent,{
        data: {icon: 'warning', text: 'Every required field should be filled!'}
      })
    }
  }
